angular.module('starter.controllers', [])
  .controller('InstitutionsCtrl', function ($scope, $state, myInsService) {
      var promise = myInsService.getEmployeeInstitutions(); // 同步调用，获得承诺接口
      promise.then(function (data) {  // 调用承诺API获取数据 .resolve
          $scope.EmployeeInstitution = data;
      });
  })
  .controller('DetailCtrl', function ($scope, $state, $stateParams, myInsService, cacheService, $timeout) {
      $scope.tips = false;
      //新增的医院
          ...
          if (!error) {
              ...
              if ($scope.Institution.Childrens.length == 0) {
                  ...
                  for (var i = 0; i < $scope.Institution.Childrens.length; i++) {
                      if ($scope.Institution.Childrens[i].Name == $scope.CreateInstitution.Name || $scope.Institution.Parent.Id == $scope.CreateInstitution.Id) {
                          break;
                      }
  ...
  .controller('SelectInsCtrl', function ($scope, $state, $timeout, cacheService, myInsService) {
      //选中的root机构     
      ...
      $scope.pushtochildren = function (model) {
          ...
          if ($scope.rootIns.Childrens.length == 0) {
              ...
              for (var i = 0; i < $scope.rootIns.Childrens.length; i++) {
                  if ($scope.rootIns.Childrens[i].Name == model.Name || $scope.rootIns.Parent.Id == model.Id) {
                      break;
                  }
  ...
  .controller('SelectLocationCtrl', function ($scope, $state, cacheService, locationService, $stateParams) {
